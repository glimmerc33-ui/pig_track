...
class BYTETracker:
    ...
    def update(self, results, img: Optional[np.ndarray] = None, feats: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        for itracked, idet in matches:
            ...
            # ==== 新增：匹配成功 → 更新 id_bank 特征 ====
            # 只有在 BOTSORT（有 encoder / id_bank）且当前 det 含外观特征时才做
            f = getattr(det, "curr_feat", None)
            if hasattr(self, "id_bank") and getattr(self, "encoder", None) is not None and f is not None:
                e = self.id_bank.get(track.track_id)
                if e is None:
                    self.id_bank[track.track_id] = {'feat': f, 'age': 0}
                else:
                    e['feat'] = self.bank_momentum * e['feat'] + (1.0 - self.bank_momentum) * f
                    e['feat'] /= (np.linalg.norm(e['feat']) + 1e-12)
                    e['age'] = 0
        ...
        # Step 4: Init new stracks  （回认优先，再新开）
        for inew in u_detection:
            ...
            # 1) 先尝试全局身份库回认（仅在 BOTSORT + with_reid 时有效）
            if hasattr(self, "id_bank") and getattr(self, "encoder", None) is not None and f is not None:
                best_id, best_dist = None, 1e9
                for pid, info in self.id_bank.items():
                    dist = 1.0 - float(np.dot(f, info['feat']))  # 余弦距离
                    if dist < best_dist:
                        best_dist, best_id = dist, pid
                if best_dist < getattr(self, "tau_reid_create", 0.30):
                    assigned_id = best_id
                    # bank里这个 ID 的代表特征做滑动平均更新
                    info = self.id_bank[assigned_id]
                    info['feat'] = self.bank_momentum * info['feat'] + (1.0 - self.bank_momentum) * f
                    info['feat'] /= (np.linalg.norm(info['feat']) + 1e-12)
                    info['age'] = 0

            # 2) 激活 YOLO 轨迹对象
            det.activate(self.kalman_filter, self.frame_id)

            # 3) 决定 track_id：回认成功则复用；否则新开并入库
            if assigned_id is not None:
                det.track_id = assigned_id
            else:
                # 此处会走 STrack.next_id() 的递增；只是把新 ID 放进 bank
                if hasattr(self, "id_bank") and getattr(self, "encoder", None) is not None and f is not None:
                    self.id_bank[det.track_id] = {'feat': f, 'age': 0}
        ...
        # 新增：bank 老化清理（每帧+1，超期剔除）
        if hasattr(self, "id_bank"):
            for pid in list(self.id_bank.keys()):
                self.id_bank[pid]['age'] += 1
                if self.id_bank[pid]['age'] > getattr(self, "bank_max_age", 3000):
                    self.id_bank.pop(pid, None)
